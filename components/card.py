...
from config.config import format_value ,format_value_STR
...
def get_figure(chart_type, config, path, index=0, barmode_group=False):
    data = config['data']
    cols = config['columns']
    ...
    custom_colors = ['#3B8AD9','#F18226', '#FFDB69', '#7BC0F7', '#00ACC1', '#DBC8FF', '#80BE86', '#909CC2', '#707070', '#73B8C8', '#E58FAE', '#E3554C']
    ...
    if chart_type == 'Pie':
        ...
        # Choose the correct formatting function based on the path
        if path == '/STR':
            fig_params['text'] = data[cols['y']].apply(format_value_STR)
        else:
    ...
    if chart_type == 'Line':
        formatted_text = data[cols['y']].apply(format_value_STR if path == '/STR' else format_value)
        if color:
            fig.update_traces(
...
def get_figure_STR(chart_type, config, path, barmode_group=False):
    data = config['data']
    cols = config['columns']
    color = cols.get('color', None)
    line = cols.get('line', None)
    ...
    if chart_type == 'Line':
        formatted_text = data[cols['y']].apply(format_value_STR)
        if color:
            fig.update_traces(
                mode='lines+markers+text',
                text=formatted_text,
    ...
    fig.update_layout(
        xaxis=dict(
            ...
            tickformat="%Y-%m-%d",  # Format the x-axis labels
            automargin=True,  # Ensure enough margin for the rotated labels
            dtick=1,  # Ensure that every tick mark is displayed on the x-axis
            tickmode='array',  # Use an array to define the tick values
            tickvals=data[cols['x']],  # Provide the tick values
            ticktext=data[cols['x']],  # Provide the tick labelsfo
        ),
        yaxis=dict(title=None, tickfont=dict(size=10)),
        margin=dict(l=10, r=10, t=10, b=10),
        height=320,
...
def create_card(title, index, prefix, default_switch_value, default_chart_type, path=None, filter_label=None, filter_id=None, filter_options=None, default_filter_value=None, barmode_group=False, show_switch=True):
    ...
    return html.Div([
        dbc.Card(
            dbc.CardBody([
                dbc.Row([
                    dbc.Col(html.H6(title, className="Cards-title"), width=6),
